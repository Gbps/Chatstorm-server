...
abstract class Room implements ActiveRecordInterface
{
    ...
    /**
     * The value for the creatoruserid field.
     * @var        int
     */
    ...
    /**
     * Get the [creatoruserid] column value.
     *
     * @return int
     */
    ...
    /**
     * Set the value of [creatoruserid] column.
     *
     * @param  int $v new value
     * @return $this|\Room The current object (for fluent API support)
     */
    public function setCreatoruserid($v)
    {
        ...
        if ($this->creatoruserid !== $v) {
            $this->creatoruserid = $v;
    ...
    public function setTopic($v)
    {
        ...
        if ($this->topic !== $v) {
            $this->topic = $v;
            $this->modifiedColumns[RoomTableMap::COL_TOPIC] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : RoomTableMap::translateFieldName('Creatoruserid', TableMap::TYPE_PHPNAME, $indexType)];
            $this->creatoruserid = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : RoomTableMap::translateFieldName('Topic', TableMap::TYPE_PHPNAME, $indexType)];
            $this->topic = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : RoomTableMap::translateFieldName('Timeout', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(RoomTableMap::COL_CREATORUSERID)) {
            $modifiedColumns[':p' . $index++]  = 'CreatorUserId';
        }
        if ($this->isColumnModified(RoomTableMap::COL_TOPIC)) {
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'CreatedDate':
                        ...
                        break;
                    case 'CreatorUserId':
                        $stmt->bindValue($identifier, $this->creatoruserid, PDO::PARAM_INT);
                        break;
                    case 'Topic':
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(RoomTableMap::COL_CREATORUSERID)) {
            $criteria->add(RoomTableMap::COL_CREATORUSERID, $this->creatoruserid);
        }
        if ($this->isColumnModified(RoomTableMap::COL_TOPIC)) {
            $criteria->add(RoomTableMap::COL_TOPIC, $this->topic);
        }
        if ($this->isColumnModified(RoomTableMap::COL_TIMEOUT)) {
    ...
    {
        $pks = array();
        $pks[0] = $this->getRoomid();
        $pks[1] = $this->getCreatoruserid();
    ...
    {
        $this->setRoomid($keys[0]);
        $this->setCreatoruserid($keys[1]);
    }
    ...
    {
        return (null === $this->getRoomid()) && (null === $this->getCreatoruserid());
    }
